getRatingPoints <- 
function(z,x,m,k,baseRef=NULL,ni=1000,p,plot=F) {
  ...
  # resampling according distance (for r and k)
  ...
  pq <- matrix(NA,nrow=length(p),ncol=6,dimnames=list(NULL,c('p','a','r','k','v','q')))
  pq[,'p'] <- p
  for (i in 1:length(p)) {
    pp <- b+p[i]
    ix <- zz1<pp
    pq[i,'a'] <- sum((pp-zz1[ix])*dx1)
    ix <- zz2<pp
    pq[i,'r'] <- sum(ix)*dr
    pq[i,'k'] <- mean(kk[ix])
  }
  pq[,'v'] <- sqrt(m)*(pq[,'a']/pq[,'r'])^(2/3)*pq[,'k']
  pq[,'q'] <- pq[,'v']*pq[,'a'] 
  ...
  if(plot) {
    ...
    points(x=rep(x[min(baseRef)],length(p)),y=b+p,pch='-',cex=2)
    text(x=rep(x[min(baseRef)],length(p))+diff(par('usr')[1:2])/50,y=b+p+diff(par('usr')[3:4])/50,labels=paste(p,'m'),pos=4,offset=0)
  }
